...
$id = $_GET['id'];
$q  = mysqli_query($conn, "SELECT * FROM unit_kerja WHERE id=$id");
$data = mysqli_fetch_assoc($q);

if (isset($_POST['update'])) {
    $nama = mysqli_real_escape_string($conn, $_POST['nama_unit']);
    mysqli_query($conn, "UPDATE unit_kerja SET nama_unit='$nama' WHERE id=$id");
...
<h1 class="text-xl font-bold mb-4">Edit Unit Kerja</h1>

<form method="post" class="bg-white p-4 rounded shadow max-w-md">
    <label class="block mb-2 text-sm">Nama Unit Kerja</label>
    <input type="text" name="nama_unit" class="border p-2 w-full mb-4" value="<?php echo $data['nama_unit']; ?>" required>
